...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return handleServerError(error, 'Failed to fetch roadmaps');
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const roadmap = await prisma.roadmap.create({
      data: { title, slug, description, icon, color, category, order: order || 0, published: published || false },
    });

    return NextResponse.json(roadmap);
  } catch (error: any) {
    return handleServerError(error, 'Failed to create roadmap');
  }
}
